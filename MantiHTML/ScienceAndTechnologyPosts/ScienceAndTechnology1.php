<?php
    $error= '';
    $str=file_get_contents('ScienceAndTechnologyTitle1.json');
    ...
    if($type==1){
        $sql="SELECT * FROM postcomment inner join ratings using (CommentID)  WHERE  Thread_ID=".$ID;
    ...
    if($type==0){
        $sql="SELECT * FROM postcomment inner join ratings using (CommentID)  WHERE Posts_ID=".$ID;
        $result=$conn->query($sql);
        $Comments='[';
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
 ...
 $Comments.='{"Comment_ID":'.$row['CommentID'].','.'"Comment_message":'.'"'.$row['Comment_message'].'"'.','.'"Comment_Date":'.'"'.$row['Comment_Date'].'"'.','.'"Comment_Rating":"'.$ratings[$index].'"},';
    ...
    if(isset($_POST['submit2'])){
        ...
        unset($_POST);
        $conn->close();
        header("Location: ScienceAndTechnology1.php");  
        }
...
<!DOCTYPE html>
    <html>
        ...
        <body>
            ...
            <form name="form" Method="POST" action="ScienceAndTechnology1.php" style="width: 100%">
            <div id="pageContent">
            ...
            $.getJSON("../../MantiHTML/ScienceAndTechnologyPosts/ScienceAndTechnologyData1.json", function(obj){
            ...
            $.getJSON("../../MantiHTML/ScienceAndTechnologyPosts/comments.json", function(obj){
                $.each(obj, function(key,value){
